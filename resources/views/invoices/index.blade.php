...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div class="flex gap-3">
            <form action="{{ route('invoices.generate') }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin generate invoice bulan ini untuk SEMUA customer aktif?');">
                @csrf
                <button type="submit" class="inline-flex items-center gap-2 px-5 py-2.5 bg-gray-700 hover:bg-gray-600 text-gray-300 font-semibold rounded-xl transition-all border border-gray-600">
            ...
            <a href="{{ route('invoices.create') }}" class="inline-flex items-center gap-2 px-6 py-2.5 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white font-semibold rounded-xl transition-all duration-200 shadow-lg shadow-purple-500/25">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="bg-gray-800/50 backdrop-blur-xl rounded-2xl border border-gray-700/50 p-6">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-2xl font-bold text-emerald-400">Rp {{ number_format($stats['paid'], 0, ',', '.') }}</p>
                </div>
            </div>
        </div>
        <div class="bg-gray-800/50 backdrop-blur-xl rounded-2xl border border-gray-700/50 p-6">
    ...
    <div class="bg-gray-800/50 backdrop-blur-xl rounded-2xl border border-gray-700/50 p-6">
        <form action="" method="GET" class="flex flex-wrap gap-4 items-end">
            <div class="flex-1 min-w-[250px]">
                ...
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Invoice Number or Customer Name..." 
            ...
            <div class="min-w-[150px]">
                ...
                <select name="status" class="w-full bg-gray-700/50 border border-gray-600 rounded-xl px-4 py-2.5 text-white focus:ring-2 focus:ring-purple-500 focus:border-transparent">
            ...
            <button type="submit" class="px-6 py-2.5 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white font-semibold rounded-xl transition-all duration-200">
                Filter
            </button>
            <a href="{{ route('invoices.index') }}" class="px-6 py-2.5 bg-gray-700 hover:bg-gray-600 text-white font-semibold rounded-xl transition-all duration-200">
                Reset
            </a>
        </form>
    </div>
            ...
            <table class="w-full">
                <thead class="bg-gray-900/50">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider">Due Date</th>
                ...
                <tbody class="divide-y divide-gray-700/50">
                    ...
                    <tr class="hover:bg-gray-700/30 transition-colors">
                        ...
                        <td class="px-6 py-4">
                            ...
                            @endphp
                            <span class="px-3 py-1 rounded-full text-sm font-medium bg-{{ $statusColor }}-500/20 text-{{ $statusColor }}-400">
                                {{ ucfirst($invoice->status) }}
                            </span>
                        </td>
                        <td class="px-6 py-4 text-right">
                            <a href="{{ route('invoices.show', $invoice) }}" class="inline-flex items-center gap-1 px-3 py-1.5 bg-purple-600/20 hover:bg-purple-600/40 text-purple-400 rounded-lg transition-colors">

...
<div class="space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-3xl font-bold bg-gradient-to-r from-yellow-400 to-lime-400 bg-clip-text text-transparent">SLA Management</h1>
            <p class="text-gray-400 mt-1">Kelola Service Level Agreement</p>
        </div>
        <a href="{{ route('sla.create') }}" class="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-yellow-600 to-lime-600 hover:from-yellow-500 hover:to-lime-500 text-white font-semibold rounded-xl transition-all duration-200 shadow-lg shadow-yellow-500/25">
            ...
            <table class="w-full">
                <thead class="bg-gray-900/50">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-center text-xs font-semibold text-gray-400 uppercase">First Response</th>
                ...
                <tbody class="divide-y divide-gray-700/50">
                    ...
                    <tr class="hover:bg-gray-700/30 transition-colors">
                        <td class="px-6 py-4">
                            ...
                            @if($policy->description)<p class="text-sm text-gray-400">{{ Str::limit($policy->description, 50) }}</p>@endif
                        </td>
                        <td class="px-6 py-4">
                            <span class="px-3 py-1 rounded-full text-xs font-medium bg-{{ $policy->priority_color }}-500/20 text-{{ $policy->priority_color }}-400">{{ $policy->priority_label }}</span>
                        </td>
                        <td class="px-6 py-4 text-center text-white font-medium">{{ $policy->first_response_hours }}h</td>
                        <td class="px-6 py-4 text-center text-white font-medium">{{ $policy->resolution_hours }}h</td>
                        <td class="px-6 py-4">
                            <span class="px-3 py-1 rounded-full text-xs font-medium {{ $policy->is_active ? 'bg-emerald-500/20 text-emerald-400' : 'bg-gray-500/20 text-gray-400' }}">
                                {{ $policy->is_active ? 'Aktif' : 'Nonaktif' }}
                            </span>
                        </td>
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-2">
                                <a href="{{ route('sla.edit', $policy) }}" class="p-2 bg-blue-600/20 hover:bg-blue-600/40 text-blue-400 rounded-lg transition-colors">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                </a>
                                <form action="{{ route('sla.destroy', $policy) }}" method="POST" class="inline" onsubmit="return confirm('Hapus SLA ini?')">
                                    @csrf
                                    @method('DELETE')
                    ...
                    @empty
                    <tr><td colspan="6" class="px-6 py-12 text-center text-gray-400">Belum ada SLA policy.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>

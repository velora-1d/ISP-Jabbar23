<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <div>
                <h2 class="font-bold text-2xl text-white leading-tight">{{ __('Manajemen Karyawan') }}</h2>
                <p class="text-gray-400 text-sm mt-1">Monitor status real-time teknisi lapangan</p>
            </div>
            <a href="{{ route('technicians.create') }}" class="inline-flex items-center px-5 py-2.5 text-sm font-semibold text-white bg-gradient-to-r from-blue-600 to-cyan-500 rounded-xl hover:from-blue-700 hover:to-cyan-600 focus:ring-4 focus:ring-blue-500/30 transition-all duration-200 shadow-lg shadow-blue-500/25">
    ...
    <div class="py-8">
        <div class="max-w-full sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                @endphp
                <div class="rounded-2xl bg-gray-800 border {{ $statusColors[$status] ?? 'border-gray-700' }} overflow-hidden hover:shadow-xl transition-all duration-300">
                    <div class="p-6">
                        <!-- Header with photo/avatar and status -->
                        <div class="flex items-start justify-between mb-4">
                            <div class="flex items-center space-x-4">
                                @if($tech->photo)
                                <img src="{{ $tech->photo_url }}" alt="{{ $tech->name }}" class="w-14 h-14 rounded-full object-cover ring-2 ring-gray-700">
                                @else
                                <div class="w-14 h-14 rounded-full bg-gradient-to-br from-blue-500 to-cyan-500 flex items-center justify-center text-white font-bold text-lg">
                            ...
                            <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium border {{ $badgeColors[$status] ?? $badgeColors['off_duty'] }}">
                                @if($status === 'available')
                        ...
                        <div class="mb-4 p-4 rounded-xl bg-gray-900/70 border border-amber-500/20">
                            ...
                            <div class="flex items-start justify-between {{ !$loop->last ? 'mb-3 pb-3 border-b border-gray-700' : '' }}">
                                <div class="flex-1">
                                ...
                                <div class="ml-3">
                                    ...
                                    @endphp
                                    <span class="px-2 py-1 rounded-full text-xs font-medium {{ $taskStatusColors[$task->status] ?? 'bg-gray-500/20 text-gray-400' }}">
                                        {{ $task->status_label }}
                                    </span>
                                </div>
                            </div>
                            ...
                            <p class="text-xs text-gray-500 mt-2">+{{ $tech->currentTasks->count() - 2 }} tugas lainnya</p>
                            @endif
                        </div>
                        ...
                        <div class="mb-4 p-4 rounded-xl bg-gray-700/50 border border-gray-600 text-center">
                            <svg class="w-8 h-8 text-gray-500 mx-auto mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636"></path></svg>
                        ...
                        <div class="text-[11px] text-gray-500 mb-4 flex items-center bg-gray-900/30 px-3 py-1.5 rounded-lg border border-gray-700/50">
                            <span class="mr-2">🏁</span>
                            <span class="flex-1">Terakhir: <strong>{{ $tech->completedCustomers->first()->name }}</strong></span>
                            <span class="text-[10px]">{{ $tech->completedCustomers->first()->updated_at->diffForHumans() }}</span>
                        </div>
                        @endif
                        ...
                        <div class="flex items-center justify-between pt-4 border-t border-gray-700">
                            <div class="flex items-center space-x-2">
                                <a href="{{ route('technicians.show', $tech) }}" class="p-2 rounded-lg hover:bg-gray-700 transition-colors text-gray-400 hover:text-white" title="Detail">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                                </a>
                                <a href="{{ route('technicians.edit', $tech) }}" class="p-2 rounded-lg hover:bg-gray-700 transition-colors text-gray-400 hover:text-white" title="Edit">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                </a>
                                @if($tech->phone)
                                <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $tech->phone) }}" target="_blank" class="p-2 rounded-lg hover:bg-emerald-500/20 transition-colors text-gray-400 hover:text-emerald-400" title="WhatsApp">
                ...
                <div class="col-span-full">
                    <div class="rounded-2xl bg-gray-800 border border-gray-700 p-12 text-center">
                        ...
                        <a href="{{ route('technicians.create') }}" class="inline-flex items-center px-4 py-2 text-sm font-semibold text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition-colors">
                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path></svg>
                            Tambah Teknisi
                        </a>
                    </div>

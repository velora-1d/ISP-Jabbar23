...
<div class="space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-3xl font-bold bg-gradient-to-r from-pink-400 to-rose-400 bg-clip-text text-transparent">Referral Program</h1>
            <p class="text-gray-400 mt-1">Kelola program referral pelanggan</p>
        </div>
        <a href="{{ route('referrals.create') }}" class="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-pink-600 to-rose-600 hover:from-pink-500 hover:to-rose-500 text-white font-semibold rounded-xl transition-all shadow-lg shadow-pink-500/25">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div class="bg-gray-800/50 backdrop-blur-xl rounded-2xl border border-gray-700/50 p-6 flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-2xl font-bold text-emerald-400">Rp {{ number_format($stats['total_rewards'], 0, ',', '.') }}</p>
            </div>
        </div>
    </div>
            ...
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-700/50">
                    ...
                    <tr class="hover:bg-gray-700/30 transition-colors">
                        <td class="px-6 py-4">
                            <code class="px-2 py-1 bg-gray-700 text-pink-400 rounded text-sm font-mono">{{ $referral->code }}</code>
                        </td>
                        <td class="px-6 py-4 text-white">{{ $referral->referrer->name ?? '-' }}</td>
                        <td class="px-6 py-4 text-gray-300">{{ $referral->referred->name ?? '-' }}</td>
                        <td class="px-6 py-4 text-right text-white font-medium">Rp {{ number_format($referral->reward_amount, 0, ',', '.') }}</td>
                        <td class="px-6 py-4">
                            <span class="px-3 py-1 rounded-full text-xs font-medium bg-{{ $referral->status_color }}-500/20 text-{{ $referral->status_color }}-400">{{ $referral->status_label }}</span>
                        </td>
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-2">
                                ...
                                <form action="{{ route('referrals.qualify', $referral) }}" method="POST" class="inline">
                                    @csrf
                                    <button type="submit" class="p-2 bg-blue-600/20 hover:bg-blue-600/40 text-blue-400 rounded-lg transition-colors" title="Qualify">
                                ...
                                <form action="{{ route('referrals.pay', $referral) }}" method="POST" class="inline">
                                    @csrf
                                    <button type="submit" class="p-2 bg-emerald-600/20 hover:bg-emerald-600/40 text-emerald-400 rounded-lg transition-colors" title="Pay">
                                ...
                                @endif
                                <form action="{{ route('referrals.destroy', $referral) }}" method="POST" class="inline" onsubmit="return confirm('Hapus?')">
                                    @csrf
                                    @method('DELETE')
                    ...
                    @empty
                    <tr><td colspan="6" class="px-6 py-12 text-center text-gray-400">Belum ada referral.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
